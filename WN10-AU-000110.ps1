...
if ((Get-ItemProperty -Path $regPath -Name $regName -ErrorAction SilentlyContinue).$regName -ne $expectedValue) {
    ...
    Write-Host "Fix applied: DisableHTTPPrinting is now set to 1 (HTTP Printing Disabled)" -ForegroundColor Cyan
} else {
    Write-Host "Compliant: DisableHTTPPrinting is correctly set to 1 (HTTP Printing Disabled)" -ForegroundColor Green
}
